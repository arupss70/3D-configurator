...
const Hero3D = () => {
  ...
  useEffect(() => {
    ...
    const createCartQuery = `mutation { cartCreate { cart { id checkoutUrl } userErrors { field message } } }`;
  ...
  useEffect(() => {
    ...
    if (baseTypeFromId) {
      ...
      const item = {
        ...
        quantity: 1,
        description: { base: baseTypeFromId.value },
      };
      const updatedLineItems = [...state.lineItem, item];
  ...
  return (
    <div className="flex flex-col h-full 2xl:h-auto bg-gray-200 relative">
      <div id="model-viewer-container" style={{ display: "none" }}></div>
      {/* Loading Overlay */}
      {state.isLoading && (
      ...
      <div
        className="flex flex-col md:flex-row w-full flex-grow h-full lg:mb-3 overflow-hidden"
        ref={scrollToTopRef}
      >
        <div className="w-full md:w-80 lg:w-96 p-4 md:p-6 bg-gray-200 shadow-2xl rounded-3xl flex-shrink-0">
          <div className="flex justify-center mb-4 lg:mb-2">
            <img
              src={logo}
              alt="Logo"
              className="w-24 md:w-36 h-auto transition-transform transform hover:scale-105"
            />
          </div>
          <div>
            ...
            <div className="flex justify-center mb-2">
              ...
              >
                <button
                  ...
                  className={`px-4 py-3 rounded-full whitespace-nowrap transition duration-300 md:text-sm lg:text-lg ${
                    state.activeView === "AR"
            ...
            <div className="flex flex-col space-y-4 xl:space-y-2">
              <div className="flex justify-end mt-4 gap-3">
                ...
                <button
                  onClick={() =>
                    resetAll(state, dispatch, toast, setVariantID, setIdNull)
                  }
            ...
            <div className="flex flex-wrap justify-evenly md:justify-start lg:justify-evenly space-x-4 mt-4">
              ...
              <button
                onClick={() =>
                  removeLevel(state, dispatch, toast, setVariantID, setIdNull)
                }
